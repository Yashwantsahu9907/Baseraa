...
const RoomOwnerDashboard = () => {
  ...
  const fetchData = async () => {
    try {
        ...
        setBookings(bookingsRes.data.filter(b => b.propertyModel === 'RoomListing'));
    } catch (error) {
    ...
    <div className="min-h-screen bg-slate-50 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
          <div>
            <h1 className="text-3xl font-black text-slate-900 tracking-tight">Room Owner Portal</h1>
            <p className="mt-1 text-slate-500 font-medium tracking-wide">Manage your student accommodations and inquiries.</p>
          </div>
          <Link
            to="/owner/add-room"
            className="flex items-center px-6 py-3 bg-primary-600 text-white rounded-2xl font-bold shadow-xl shadow-primary-100 hover:bg-primary-700 transition-all hover:-translate-y-0.5 active:scale-95"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            <h3 className="text-xl font-black text-slate-900 uppercase tracking-widest flex items-center">
              <HomeIcon className="w-5 h-5 mr-3 text-primary-500" /> Active Listings
            </h3>
            ...
            {listings.length === 0 ? (
                <div className="glass p-12 rounded-3xl text-center border-dashed border-2 border-slate-300">
                    ...
                    <p className="mt-4 text-slate-500 font-bold">No properties listed yet.</p>
                </div>
            ) : (
                    ...
                    {listings.map(listing => (
                        <div key={listing._id} className="glass rounded-3xl overflow-hidden border border-white bg-white/40 backdrop-blur-md shadow-lg shadow-slate-200/50 group">
                            <div className="relative h-44">
                                <img src={listing.images?.[0]?.url} alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
          ...
          {/* Sidebar: Recent Booking Requests */}
          <div className="lg:col-span-1 space-y-8">
            <h3 className="text-xl font-black text-slate-900 uppercase tracking-widest flex items-center">
              <Bell className="w-5 h-5 mr-3 text-amber-500" /> Student Requests
            </h3>
                    ...
                    <div key={booking._id} className="glass p-5 rounded-3xl bg-white shadow-xl shadow-slate-200/40 border border-slate-50">
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <div className="text-right">
                                <p className="text-xs font-black text-slate-900">₹{booking.amount}</p>
                                <p className="text-[8px] text-slate-400 font-bold uppercase">DEPOSIT</p>
                            </div>
                        </div>
